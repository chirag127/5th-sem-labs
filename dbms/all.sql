...
select count(*) from product_master where sell_price>=1500;create table sales_master(
...
create view salesman_view as select * from sales_master where sal_amt < 3500;

-- 8. Create a view client_view on client_master and rename the columns as name, add1, add2, city, pcode, state respectively.

create view client_view as select  name, city, pincode as pcode, state from client_master;
